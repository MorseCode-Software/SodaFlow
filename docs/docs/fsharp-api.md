...
There is no generated API reference for `SodiumFRP.FSharp` on this site. That is deliberate,
...
**The assembly carries no documentation comments.** `Sodium.FSharp.Frp` has
...
readable: fourteen files under
[`dotnet/src/Sodium/FSharp/Sodium.FSharp.Frp`](https://github.com/SodiumFRP/sodium/tree/master/dotnet/src/Sodium/FSharp/Sodium.FSharp.Frp).
...
// Suffixed aliases, from the [<AutoOpen>] module in Sodium.fs.
let c = s |> holdS 0
let l = c |> listenC (printfn "%d")
```

The suffix names the type operated on — `S` stream, `C` cell, `B` behavior, `L` listener,
`T` transaction. `Sodium.fs` is the single file listing every alias, and it doubles as the best
available index of the F# API.

## What it would take to fix this

1. Write `///` comments across `Sodium.FSharp.Frp`. This is the real work, and it is
