...
turns a stream into a cell that remembers the most recent value. `ListenStrong` subscribes.
...
Note the `0` before `2`. `ListenStrong` on a cell fires immediately with the cell's current value,
...
`Map`, `Hold`, `Snapshot`, `Filter`, `Merge`, `ListenStrong` and the rest are extension methods in
...
`Stream.map`, `Cell.listenStrong`, `StreamSink.send` — and a set of suffixed aliases in an
`[<AutoOpen>]` module, so `open SodaFlow` alone gives you `mapS`, `listenStrongC`, `sendS`,
