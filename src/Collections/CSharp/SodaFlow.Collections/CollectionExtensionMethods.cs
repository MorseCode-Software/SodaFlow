...
///     optional type back on top. See <see cref="ReactiveCollection{TKey,TId,TState}" />.
/// </remarks>
[PublicAPI]
public static class CollectionExtensionMethods
{
    ...
    ///         <see cref="IReactiveCollection{TKey,TId,TState}.KeysCell" />.
    ///     </para>
    /// </remarks>
    [MethodImpl(MethodImplOptions.NoInlining)]
    public static Cell<Maybe<TState>> StateCell<TKey, TId, TState>(
        this IReactiveCollection<TKey, TId, TState> collection,
        TKey key)
        where TKey : notnull
        where TId : notnull =>
        collection.Root.StateCellImpl(
            key,
    ...
    [MethodImpl(MethodImplOptions.NoInlining)]
    public static Cell<Maybe<TId>> IdentityCell<TKey, TId, TState>(
        this IReactiveCollection<TKey, TId, TState> collection,
        TKey key)
        where TKey : notnull
        where TId : notnull =>
        collection.Root.ShapeCell.Map(identities => identities.TryGetValue(key));
    ...
    [Pure]
    [MethodImpl(MethodImplOptions.NoInlining)]
    public static Maybe<Entry<TId, TState>> Lookup<TKey, TId, TState>(
        this CollectionSnapshot<TKey, TId, TState> snapshot,
        TKey key)
        where TKey : notnull
        where TId : notnull =>
        snapshot.TryGetEntry(key, out Entry<TId, TState>? entry) && entry is not null
            ? Maybe.Some(entry)
            : Maybe<Entry<TId, TState>>.None;
    ...
    [Pure]
    [MethodImpl(MethodImplOptions.NoInlining)]
    public static Maybe<Maybe<TState>> ChangeFor<TKey, TId, TState>(
        this CollectionChange<TKey, TId, TState> change,
        TKey key)
        where TKey : notnull
        where TId : notnull
    {
    ...
    /// <remarks>
    ///     <see cref="IOrderedKeys{TKey,TId,TState}.IndexOf" /> itself answers -1, following the
    ...
    [Pure]
    [MethodImpl(MethodImplOptions.NoInlining)]
    public static Maybe<int> IndexOfMaybe<TKey, TId, TState>(
        this IOrderedKeys<TKey, TId, TState> keys,
        TKey key)
        where TKey : notnull
        where TId : notnull
    {
