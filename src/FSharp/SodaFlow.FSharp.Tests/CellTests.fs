...
module private Types =
    type Test(initialValue: int) =
        member val Value = sinkC initialValue
...
type ``Cell Tests``() =
    ...
    member _.``Test Loop``() =
        task {
            ...
            let output1 = List<_>()
            let output2 = List<_>()
            let l = c |> listenStrongC output1.Add
            let l2 = c |> updatesC |> listenStrongS output2.Add
            s |> sendS 5
            s |> sendS 7
            l2 |> unlistenL
            l |> unlistenL
            do! Expect.Sequence([ 15; 25; 35 ], output1)
            do! Expect.Sequence([ 25; 35 ], output2)
        }
    ...
    member _.``Test Lift Simultaneous Updates``() =
        task {
            ...
            let l = (cellSink, cell) |> lift2C (+) |> updatesC |> listenStrongS out.Add
            cellSink |> sendC 2
            cellSink |> sendC 7
            l |> unlistenL
            do! Expect.Sequence([ 6; 21 ], out)
        }
    ...
    member _.``Test Map With SwitchC``() =
        task {
            ...
            let c =
                v |> mapC ((Seq.map (fun o -> o.Value)) >> liftAllC id) |> mapC id |> switchC
    ...
    member _.``Test Lift Cells In SwitchC``() =
        task {
            ...
            let l = r |> switchC |> listenStrongC out.Add
            s |> sendC 2
            s |> sendC 4
            l |> unlistenL
        }
    ...
    member _.``Test Lazy Cell Creation``() =
        task {
            ...
            s |> sendS 1
            let l = c |> switchC |> listenStrongC out.Add
            s |> sendS 3
            s |> sendS 5
            l |> unlistenL
            do! Expect.Sequence([ 1; 3; 5 ], out)
        }
    ...
    member _.``Test Cell Values With Previous``() =
        task {
            ...
            let l =
                runT (fun () ->
                    let r =
                        (c |> updatesC |> snapshotC c (fun c p -> (c, Some p)),
                         constantLazyC (c |> sampleLazyC) |> valuesC |> mapS (fun c -> (c, None)))
                        |> orElseS

                    r |> listenStrongS out.Add)
            ...
            s |> sendS 4
            l |> unlistenL
            do! Expect.Sequence([ (0, None); (1, Some 0); (2, Some 1); (3, Some 2); (4, Some 3) ], out)
        }
    ...
    member _.``Test Cell Values With Previous Having Initial Update``() =
        task {
            ...
            s |> sendS 5
            l |> unlistenL
            do! Expect.Sequence([ (1, Some 0); (2, Some 1); (3, Some 2); (4, Some 3); (5, Some 4) ], out)
        }
    ...
    member _.``Test Loop And SwitchC Error``() =
        task {
            ...
            do!
                e
                |> assertExceptionExists (fun e ->
                    Expect.Equal("ValueFactory attempted to access the Value property of this instance.", e.Message))
        }
    ...
    member _.``Test Loop And SwitchC``() =
        task {
            ...
            let _ =
                (use _l = resultCell |> listenStrongC out.Add
                 (innerCell |> sampleC).s |> sendS 5

                 innerStreamSink
                 |> sendS (Inner.create (resultCell |> sampleLazyC |> Lazy.map (fun v -> v - 1)))

                 (innerCell |> sampleC).s |> sendS 7)

            do! Expect.Sequence([ 3; 5; 4; 7 ], out)
        }
