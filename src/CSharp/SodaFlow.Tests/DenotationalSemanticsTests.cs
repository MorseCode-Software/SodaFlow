...
namespace SodaFlow.Tests
{
    ...
    public class DenotationalSemanticsTests
    {
        ...
        public void Test_MapS_TestCase()
        {
            ...
            List<int> @out = RunSimulation<int>(listenStrong: s.Map(x => x + 1).ListenStrong, firings: new[] { sf });
        ...
        public void Test_Snapshot_TestCase()
        {
            ...
            List<int> @out = RunSimulation<int>(listenStrong: s1.Snapshot(c).ListenStrong, firings: new[] { s1F, s2F });
        ...
        public void Test_Merge_TestCase()
        {
            ...
            List<int> @out =
                RunSimulation<int>(
                    listenStrong: s1.Merge(s2: s2, f: (x, y) => x + y).ListenStrong,
                    firings: new[] { s1F, s2F });
        ...
        public void Test_Filter_TestCase()
        {
            ...
            List<int> @out =
                RunSimulation<int>(listenStrong: s.Filter(x => x % 2 != 0).ListenStrong, firings: new[] { sf });
        ...
        public void Test_SwitchS_TestCase() =>
            RunPermutations<char>(
                createListAndListener: createFiringsListAndListener =>
                {
                    (Stream<char> s1, Dictionary<int, Action> s1F) =
                ...
                assert: @out => CollectionAssert.AreEqual(expected: new[] { 'a', 'b', 'Y', 'Z' }, actual: @out));
        ...
        public void Test_Updates_TestCase()
        {
            ...
            List<char> @out = RunSimulation<char>(listenStrong: c.Updates().ListenStrong, firings: new[] { sf });
        ...
        public void Test_ListenC_TestCase1()
        {
            ...
            Cell<char> c = s.Hold('a');
            List<char> @out = RunSimulation<char>(listenStrong: c.ListenStrong, firings: new[] { sf });
            CollectionAssert.AreEqual(expected: new[] { 'a', 'b', 'c' }, actual: @out);
        }
        ...
        public void Test_Split_TestCase()
        {
            (Stream<IReadOnlyList<char>> s, ILookup<int, Action> sf) =
                MkStream(
                    ...
                    coalesce: (x, y) => x.Concat(y).ToArray());
        ...
        public void Test_MapC_TestCase()
        {
            ...
            Cell<int> c = s.Hold(0);
            List<int> @out = RunSimulation<int>(listenStrong: c.Map(x => x + 1).ListenStrong, firings: new[] { sf });
        ...
        public void Test_Apply_TestCase()
        {
            ...
            (Stream<Func<int, int>> s2, Dictionary<int, Action> s2F) =
                MkStream(new Dictionary<int, Func<int, int>> { { 1, x => x + 5 }, { 3, x => x + 6 } });

            Cell<Func<int, int>> cf = s2.Hold(x => x + 0);
            List<int> @out = RunSimulation<int>(listenStrong: ca.Apply(cf).ListenStrong, firings: new[] { s1F, s2F });
        ...
        public void Test_SwitchC_TestCase1() =>
            RunPermutations<char>(
                ...
                assert: @out => CollectionAssert.AreEqual(expected: new[] { 'b', 'X', 'Y', 'Z' }, actual: @out));
        ...
        public void Test_SwitchC_TestCase4() =>
            RunPermutations<char>(
                ...
                assert: @out => CollectionAssert.AreEqual(expected: new[] { 'b', 'X', 'Y', '5' }, actual: @out));
        ...
        private static (Stream<T> Stream, ILookup<int, Action> Firings) MkStream<T>(
            IReadOnlyList<(int Time, T Value)> firings,
            Func<T, T, T> coalesce)
        {
            ...
            ILookup<int, Action> f =
                firings.ToLookup(
                    keySelector: firing => firing.Time,
                    elementSelector: firing => (Action)(() => s.Send(firing.Value)));
        ...
        private static List<T> RunSimulation<T>(
            ...
            IReadOnlyList<Dictionary<int, Action>> firings) =>
            RunSimulation(
                listenStrong: listenStrong,
                firings: firings.Select(f => f.ToLookup(keySelector: p => p.Key, elementSelector: p => p.Value))
                    .ToArray());
            ...
            Action<IReadOnlyList<T>> assert)
        {
            IReadOnlyList<int> indexes =
                Enumerable.Range(
                        start: 0,
                        count: createListAndListener((fl, l) => (FiringsList: fl, ListenStrong: l)).FiringsList.Count)
                    .ToArray();
            ...
            foreach ((IReadOnlyList<(string Name, Dictionary<int, Action> Firings)> firingsList,
                     ...
                     GetPermutations(indexes)
                         .Select(ii =>
                         {
                             (IReadOnlyList<(string Name, Dictionary<int, Action> Firings)> firingsList,
                                     Func<Action<T>, IListener> listenStrong) =
                                 createListAndListener((fl, l) => (FiringsList: fl, ListenStrong: l));
                         ...
                         }))
            {
                ...
                {
                    List<T> @out =
                        RunSimulation(listenStrong: listener, firings: firingsList.Select(o => o.Firings).ToArray());
                ...
                catch
                {
                    Console.WriteLine(
                        "Test failed for ordering { " + string.Join(
                            separator: ", ",
                            values: firingsList.Select(o => o.Name)) + " }.");
        ...
        private static IReadOnlyList<IReadOnlyList<T>> GetPermutations<T>(IReadOnlyList<T> list, int length)
        {
            ...
            return GetPermutations(list: list, length: length - 1)
                .SelectMany(
                    collectionSelector: t => list.Where(e => !t.Contains(e)),
                    resultSelector: (t1, t2) => t1.Concat(new[] { t2 }).ToArray())
                .ToArray();
        }
    }
}