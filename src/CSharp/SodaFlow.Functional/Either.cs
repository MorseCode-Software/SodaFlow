...
/// <summary>
///     Constructors for the <see cref="Either{T1,T2}" /> family for which each
///     type argument to be written out.
/// </summary>
/// <remarks>
///     An either is named by all of its cases, thus a construction of one directly repeats types that the
///     code around it gives. The constructors here return a small value with a mark for its
///     position, and that value changes implicitly into the either that the code assigns or
///     returns.
/// </remarks>
[PublicAPI]
public static class Either
{
    ...
    ///     conversion at the assignment or return gives the others. Where no such conversion is
    ...
    ///     cannot specify one type argument and infer the others in one call, and the
...
///     <c>Match</c>, or with one of the helpers built on it, so that each case has to be
///     answered for.
...
public struct Either<T1, T2> : IEitherOfTwo, IEquatable<Either<T1, T2>>
{
    ...
    T IEitherOfTwo.Match<T>(Func<object?, T> onFirst, Func<object?, T> onSecond) =>
        this.Match(onFirst: v => onFirst(v), onSecond: v => onSecond(v));
    ...
    /// <remarks>
    ///     This is the only member that reads the held value, and each other member here is
    ///     expressed in terms of it. This calls one function of the set, and it calls that function
    ...
    /// </returns>
    /// <remarks>
    ///     For handing an either to something which names the same two types in the opposite sequence,
    ...
    ///     single change to make: a change means a selection of one of some reorderings,
    ///     and the name <c>Swap</c> on one of them makes the others look unavailable, and not
    ///     unnamed.
    /// </remarks>
    [Pure]
    ...
    void IEitherOfTwo.MatchVoid(Action<object?> onFirst, Action<object?> onSecond) =>
        // ReSharper disable once ReturnValueOfPureMethodIsNotUsed
        this.Upcast<IEitherOfTwo>().Match(onFirst: onFirst.ToFunc(), onSecond: onSecond.ToFunc());

    Task<T> IEitherOfTwo.MatchAsync<T>(Func<object?, Task<T>> onFirst, Func<object?, Task<T>> onSecond) =>
        this.Upcast<IEitherOfTwo>().Match(onFirst: onFirst, onSecond: onSecond);

    Task IEitherOfTwo.MatchAsyncVoid(Func<object?, Task> onFirst, Func<object?, Task> onSecond) =>
        this.Upcast<IEitherOfTwo>().MatchAsync(onFirst: onFirst.ToAsyncFunc(), onSecond: onSecond.ToAsyncFunc());
...
public struct Either<T1, T2, T3> : IEitherOfThree, IEquatable<Either<T1, T2, T3>>
{
    ...
    T IEitherOfThree.Match<T>(Func<object?, T> onFirst, Func<object?, T> onSecond, Func<object?, T> onThird) =>
        this.Match(onFirst: v => onFirst(v), onSecond: v => onSecond(v), onThird: v => onThird(v));
    ...
    void IEitherOfThree.MatchVoid(Action<object?> onFirst, Action<object?> onSecond, Action<object?> onThird) =>
        ...
        this.Upcast<IEitherOfThree>()
            .Match(onFirst: onFirst.ToFunc(), onSecond: onSecond.ToFunc(), onThird: onThird.ToFunc());

    Task<T> IEitherOfThree.MatchAsync<T>(
        Func<object?, Task<T>> onFirst,
        Func<object?, Task<T>> onSecond,
        Func<object?, Task<T>> onThird) =>
        this.Upcast<IEitherOfThree>().Match(onFirst: onFirst, onSecond: onSecond, onThird: onThird);
...
public struct Either<T1, T2, T3, T4> : IEitherOfFour, IEquatable<Either<T1, T2, T3, T4>>
{
    ...
    T IEitherOfFour.Match<T>(
        ...
        this.Match(
            onFirst: v => onFirst(v),
            onSecond: v => onSecond(v),
            onThird: v => onThird(v),
            onFourth: v => onFourth(v));
    ...
    public void MatchVoid(
        ...
        this.Match(
            ...
            onSecond: onSecond.ToFunc(),
            onThird: onThird.ToFunc(),
    ...
    Task<T> IEitherOfFour.MatchAsync<T>(
        ...
        this.Upcast<IEitherOfFour>().Match(onFirst: onFirst, onSecond: onSecond, onThird: onThird, onFourth: onFourth);
...
public struct Either<T1, T2, T3, T4, T5> : IEitherOfFive, IEquatable<Either<T1, T2, T3, T4, T5>>
{
    ...
    Task<T> IEitherOfFive.MatchAsync<T>(
        ...
        this.Upcast<IEitherOfFive>()
            .Match(onFirst: onFirst, onSecond: onSecond, onThird: onThird, onFourth: onFourth, onFifth: onFifth);
