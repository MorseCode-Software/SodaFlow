...
namespace SodaFlow
{
    internal static class BehaviorInternal
    {
        ...
        internal static BehaviorSink<T> CreateSinkImpl<T>(T initialValue, Func<T, T, T> coalesce) =>
            new BehaviorSink<T>(initialValue: initialValue, coalesce: coalesce);
    }
    ...
    public class Behavior<T>
    {
        ...
        internal Lazy<T> SampleLazy(TransactionInternal trans)
        {
            ...
            trans.Sample(() =>
            {
                s.Value = this.valueUpdate.Match(onSome: v => v, onNone: this.SampleNoTransaction);
                s.HasValue = true;
                s.Behavior = null;
            });
            ...
            IListener l =
                this.Updates()
                    .Listen(
                        target: @out.Node,
                        trans: trans1,
                        action: (trans2, v) => @out.Send(trans: trans2, a: v),
                        suppressEarlierFirings: false);

            // The order is assured without having to link the first node and the second node.
            return @out.UnsafeAttachListener(l).Coalesce(trans1: trans1, f: (left, right) => right);
        }
            ...
            IListener[] listeners)
        {
            ...
            Stream<TResult> result = pulse.Coalesce(trans1: trans, f: (x, y) => x).MapImpl(_ => recombine());
